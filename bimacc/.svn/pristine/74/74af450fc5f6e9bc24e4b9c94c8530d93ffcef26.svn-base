...
<div class="modal hide fade" id="editclaimstageRequestTypeModal{{$ClaimNoticeStages->id}}" tabindex="-1" role="dialog" aria-labelledby="createclaimstagelabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered " role="document">
    <div class="modal-content">  
      <div class="modal-header">
        <h5 class="modal-title" id="editclaimstagelabel">Send Stage Extension Request for {{$notice->claimnoticeno}}</h5>
      ...
      <div class="modal-body"> 
       ...
       <form id="createForm" action="{{ route('SendStageExtensionByRespondent.update',$ClaimNoticeStages->id) }}" 
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="date" id="stagetodate" name="stagetodate" class="form-control" required value="{{$ClaimNoticeStages->stagetodate }}" placeholder=" " disabled>
         ...
         <div class="col-md-6">
        <div class="form-group">
          <label  for="stagetodate">Stage Extension Request Date(DD-MM-YYYY)</label>
           <label for="date-picker-3" class="add-on"><i class="icon-calendar"></i>

                </label>
                <input id="date-picker-3" type="text" class="date-picker" />
          <input type="daten" id="stageextensionrequestdate" name="stageextensionrequestdate" class="form-control" required value="{{$ClaimNoticeStages->stagetodate }}" placeholder=" ">

          @if ($errors->has('stageextensionrequestdate'))
          <span class="invalid-feedback" role="alert">
            <strong>{{ $errors->first('stageextensionrequestdate') }}</strong>
          </span>
          @endif  
        </div>
      </div>
...
$("#stageextensionrequestdate").datepicker({
    changeMonth: true,
    changeYear: true,
