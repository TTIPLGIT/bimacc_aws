                ...
                <div class="card-body">
                 ...
                 <div class="row">
                  <div class="col-12 col-md-12 col-lg-12">
                    ...
                    <form  action="{{ route('misclaimpetition.store') }}" method="POST" name="new_entry" enctype="multipart/form-data" autocomplete="off" id="claimantinformationform">
           {{ csrf_field() }}
           ...
           <div class="col-md-6">
              <div class="form-group">
               <select class="form-control" name="claimant_type" id="claimant_type">
                <option value="">Select Arbitartor</option>
               @foreach ($claimant_type as $claimant_type)
                <option value="{{ $claimant_type->user_id }}" {{ $claimant_type->id ==  old('claimant_type') ? 'selected':'' }}> {{$claimant_type->firstname}}</option>
                @endforeach
              </select>
              <label class="form-control-placeholder" for="claimant_type">Select Arbitartor<span style="color: red">*</span></label> 
            </div>
          </div>
          <!--  <div class="col-md-6">
              <div class="form-group">
               <select class="form-control" name="dispute_categories_id" id="disputecategory" >
