...
<div class="container con"> 
 ...
 <div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">Resend  Video Conferencing Link</h6>
  </div>
  @foreach ($videoconferencing['video_conference_header'] as $videoconferencings)
  <div class="card-body">
    <div class="table-responsive" style="overflow-x: hidden;">
      <form  action="{{ route('videoconferencing.update',$videoconferencings->id) }}" method="POST">
        @csrf 
         @method('PUT')   
      ...
      <div class="modal-footer">
        <div class="mx-auto">
          <button type="submit" class="btn btn-success btn-space">Re-Send Invitation link</button>
           <a class="btn btn-danger btn-space" href="{{ route('videoconferencing.index') }}"> Back</a>           
        </div>
      </div> 
    </form>  
  </div>
