...
@if(($respondent_adminfees !=null && $notice->claimnoticestatus=='Arbitrator Allocated and Waiting for the Respondent Payment') )
  ...
  @if($respondent_adminfees_paid ==null)
     <form  action="{{ route('claimrespondent.payadminstrativefees_respondent') }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="col-md-4">
      <div class="form-group">
       <input type="text" id="adminstration_fees" value="{{ number_format((float)$adminfee, 2, '.', '') }}" class="form-control{{ $errors->has('adminstration_fees') ? ' is-invalid' : '' }}" name="adminstration_fees" placeholder=" " readonly>
  ...
  <div class="col-md-4">
      <div class="form-group">
       <input type="text" id="arbitrator_fees"  value="{{ number_format((float)$arbfees, 2, '.', '') }}"
 class="form-control{{ $errors->has('arbitrator_fees') ? ' is-invalid' : '' }}" name="arbitrator_fees" placeholder=" " readonly>
  ...
  <div class="col-md-4">
      <div class="form-group">
       <input type="text" id="extra_amt"  value="{{ number_format((float)$rextra_amt, 2, '.', '') }}"
 class="form-control{{ $errors->has('total_fee') ? ' is-invalid' : '' }}" name="extra_amt" placeholder=" " readonly>
  ...
  <div class="col-md-4">
      <div class="form-group">
       <input type="text" id="total_pay_amt"  value="{{ number_format((float)$rtotal_pay_amt_check, 2, '.', '') }}"
...
@endif
   ...
   <a class="btn btn-info" href="{{ route('ClaimPetion.respodentclaimpetitionshow',$notice->id) }}">Go to Claim Petition</a> 
...
@if($notice->claimnoticestatus =="Respondent Access Provided Waiting for the Acceptance")
@if(count($respondent_decision) ==0)
<div class="container" style="margin-top: 14px; max-width: 100%"  > 
  <div class="modal-content">
    <div class="modal-body">       
      <div class="row register-form" >
        <div class="mx-auto" style="display: inline-flex; width: 100%; text-align: center;">
          <form action="{{ route('claimantrespondantaccess.store',$notice->id) }}" method="POST" style="width: 100%">
            @csrf

            @if($notice->claimnoticestatus =="Respondent Access Provided Waiting for the Acceptance" && $dispute_respondent == null)
            <div  class="modal-footer" style="border: none;">
              <div class="col-md-6">
                <div class="form-group">
                  <strong>Select Status : </strong>
                  <select  name="claimnoticestatus" id="claimnoticestatus" class="form-control" onchange="status_select()">
                    <option>Select Staus</option>
                    <!-- <option value="Accepting and Admitting the Claim">Accepting and Admitting the Claim</option> -->
                    <option value="Disputing and Contesting the Claim">Contest the Claim</option>
                    <option value="Making a Counter Claim">Contest with Counter Claim</option>
                    <!-- <option value="Willing to Negotiate Directly/Mediate">Willing to 
                    Negotiate Directly/Mediate</option>
                    <option value="Directly Negotiate with the Bank">Directly Negotiate with the Bank</option>
                    <option value="Opt for Mediation">Opt for Mediation</option> -->
                  </select>
                </div>
              </div>
              <div class="col-md-6"> 
           ...
           @endif
           <div class="modal-footer" style="margin-bottom: 50px; border: none;">
            <div class="mx-auto" style="display: inline-flex;">
             
              @if($notice->claimnoticestatus =='Respondent Access Provided Waiting for the Acceptance')
              <button type="submit"  style="display: none;" class="btn btn-warning btn-space" id="update">Update and Send</button> 
             
               <a class="btn btn-primary" id="dispute_select" style="display: none;" data-toggle="modal" data-target="#respondentdisputemodal" id="make_counter" style="
              color: white;">Select Dispute</a>
               
              @endif
              @if($dispute_respondent != null && $payment_relief == null)
               <a class="btn btn-success float-right"  title="create"  data-toggle="modal" data-target="#idrespondentclaiminformationdetails" >Create Counter Claim</a>
                
               @endif
               @if($payment_relief != null)
               <a class="btn btn-success float-right"  title="create"  data-toggle="modal" data-target="#respondentpayment">Pay Registration Fee</a> 
...
@include('modals/respondantcounterclaimdetails') 
@include('modals/respondentdisputemodal') 
@include('modals/respondentpayment') 
@endif
</div>
@endif
</div>
...
<script type="text/javascript">
 ...
 {
  var inputvalue = document.getElementById("claimnoticestatus").value;

// alert (inputvalue);

  if( inputvalue==="Accepting and Admitting the Claim"){
  ...
  else if( inputvalue==="Disputing and Contesting the Claim")
  {
   $("#update").show();
   $("#make_counter").hide();
    $("#dispute_select").hide();
 ...
 else if( inputvalue==="Making a Counter Claim")
 {
  $("#update").hide();
  $("#make_counter").show();
  $("#dispute_select").show();
